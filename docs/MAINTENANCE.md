...
| **Raspberry Pi Imager `--repo`** | `imager.py`, `os-list/` | The whole write path is now theirs. `--repo` is documented, but a schema change to the OS list breaks the device picker SILENTLY — empty list, no error. Verified against v2.0.10. **Now probed daily** by `check-upstreams.py` (`check_os_list`): the list must resolve, be served as `application/json`, and the image URL inside it must still exist — Raspberry Pi removes old image paths, so a valid list can outlive its download. |
